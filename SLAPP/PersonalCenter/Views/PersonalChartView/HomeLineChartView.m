...
-(void)addYAxisViews
{
    CGFloat height = _lineChartView.bounds.size.height / 5;
    if (_dataArrOfY.count != 1) {
        height = _lineChartView.bounds.size.height / (_dataArrOfY.count - 1);
    }
    for (int i = 0;i< _dataArrOfY.count ;i++ )
    {
        ...
        leftLabel.text = [NSString stringWithFormat:@"%@%@",_dataArrOfY[i],@"%"];
...
{
    CGFloat height = 0;
    if (_dataArrOfX.count != 1) {
        height = _lineChartView.bounds.size.width /( _dataArrOfX.count - 1);
    }
...
-(void)addLinesView
{
    ...
    CGFloat height = 0;
    if (_dataArrOfY.count != 1) {
        white = _lineChartView.bounds.size.height /( _dataArrOfY.count - 1);
    }
    if (_dataArrOfX.count != 1) {
        height = _lineChartView.bounds.size.width /( _dataArrOfX.count - 1);
    }
   
    if (_dataArrOfY.count > 2) {
        //横格
...
-(void)addPointView
{
    ...
    float Xmargin = 0;
    if (_dataArrOfX.count != 1) {
       Xmargin =  width / (_dataArrOfX.count - 1 );
    }
...
//            [beizer addLineToPoint:nowPoint controlPoint1:CGPointMake((nowPoint.x+prePoint.x)/2, prePoint.y) controlPoint2:CGPointMake((nowPoint.x+prePoint.x)/2, nowPoint.y)];
        }
    }
    //显示线
...
//    修改颜色
    shapeLayer.strokeColor = kgreenColor.CGColor;
...
//            UIView * cycle = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 4, 4)];
//            cycle.layer.cornerRadius = 2;
//            cycle.backgroundColor = kgreenColor;
//            cycle.clipsToBounds = YES;
//            cycle.center = nowPoint;
//            [_lineChartView addSubview:cycle];
        }
    }
    //获取最后一个点的X值
    ...
    shadeLayer.fillColor = kgreenColor.CGColor;
...
//    shadeLayer.fillColor = [UIColor colorWithRed:245/255.0 green:166/255.0  blue:35/255.0  alpha:1].CGColor;
    ...
    gradientLayer.colors = @[(__bridge id)[UIColor colorWithRed:214/255.0 green:245/255.0  blue:235/255.0  alpha:0.7].CGColor,(__bridge id)[UIColor colorWithRed:214/255.0 green:245/255.0  blue:235/255.0  alpha:0.7].CGColor];
