...
- (UITableView *)resultTableView {
    if (!_resultTableView) {
        _resultTableView =
            [[UITableView alloc] initWithFrame:CGRectMake(0,_navHeight+20, self.view.frame.size.width, _mHeight-_navHeight-20)
...
//        _navHeight = 88;
//        _mHeight = [UIScreen mainScreen].bounds.size.height - 83 + 49;
//    }else{
//        _navHeight = 64;
//        _mHeight = [UIScreen mainScreen].bounds.size.height;
//    }
    
    _navHeight = kNav_height;
    _mHeight = kScreenHeight + 49 - kTab_height;
...
- (void)loadSearchView {
    self.searchView = [[UIView alloc] initWithFrame:CGRectMake(0, _navHeight-64 + 20,self.view.frame.size.width, 44)];
    ...
    _cancelButton = [[UIButton alloc]
        initWithFrame:CGRectMake(CGRectGetMaxX(_searchBars.frame), CGRectGetMinY(self.searchBars.frame), 65, 44)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.conversationType == ConversationType_PRIVATE) {
        ...
        info.userId = [UserModel getUserModel].im_userid;
        info.name = [UserModel getUserModel].realname;
        if ([UserModel getUserModel].avater == nil) {
            info.portraitUri = @"http://t-tcp.xslp.cn/static/images/userpic.jpg";
        }else{
            info.portraitUri = [[UserModel getUserModel].avater stringByAppendingString:@"180x180.jpg"];
        }
        [[RCIM sharedRCIM] refreshUserInfoCache:info withUserId:info.userId];
        
        
        RCUserInfo *info1 = [[RCUserInfo alloc]init];
        info1.userId = model.targetId;
        info1.name = model.name;
        
        UserModelTcp *tModel = [UserModelTcp objectForPrimaryKey:[model.targetId componentsSeparatedByString:@"-"].firstObject];
        if (tModel != nil) {
            info1.portraitUri = tModel.avater;
        }
        [[RCIM sharedRCIM] refreshUserInfoCache:info1 withUserId:info1.userId];
        
        SingleChatVC *vc = [[SingleChatVC alloc]initWithConversationType:model.conversationType targetId:model.targetId];
        vc.title = model.name;
        [self.navigationController pushViewController:vc animated:true];
        
        
    }else if (model.conversationType == ConversationType_GROUP){
        GroupModel *myModel = [GroupModel objectForPrimaryKey:model.targetId];
        TMTabbarController *tab = [[TMTabbarController alloc]init];
        if (myModel == nil) {
        ...
        if ([myModel.parentid isEqualToString:@""] || [myModel.parentid isEqualToString:@"0"]) {

            RCConversation * vcSation = [[RCIMClient sharedRCIMClient] getConversation:model.conversationType targetId:myModel.groupid];
            tab.groupModel = [[RCConversationModel alloc]initWithConversation:vcSation extend:nil];
            if (tab.groupModel.targetId == nil) {
                //todo 加提醒
            }
...
//            tab.groupModel = [[RCConversationModel alloc]initWithConversation:vcSation extend:nil];
//            if (tab.groupModel.targetId == nil) {
//                tab.groupModel.targetId = myModel.parentid;
//            }
//            [self.navigationController pushViewController:tab animated:true];
//            [tab btnClick:[tab.bgImageView viewWithTag:1001]];
            ThemeChatVC *tvc = [[ThemeChatVC alloc]initWithConversationType:model.conversationType targetId:myModel.groupid];
            tvc.title = myModel.group_name;
