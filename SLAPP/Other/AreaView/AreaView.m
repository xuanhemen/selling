...
#define MULPITLE [[UIScreen mainScreen] bounds].size.width / ScreenWidth
...
- (void)creatBaseUI
{
    ...
    _areaWhiteBaseView = [[UIView alloc]initWithFrame:CGRectMakes(0, ScreenHeight, ScreenWidth, 420)];
    ...
    _areaScrollView.contentSize = CGSizeMake(ScreenWidth * MULPITLE * 1, 300 * MULPITLE);
    ...
    for (int i = 0; i < 3; i++) {
        UITableView *area_tableView = [[UITableView alloc]initWithFrame:CGRectMakes(ScreenWidth * i, 0, ScreenWidth, 300) style:UITableViewStylePlain];
...
- (void)areaBtnAction:(UIButton *)btn
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        _areaScrollView.contentOffset = CGPointMake(ScreenWidth * MULPITLE * (btn.tag - 100), 0);
    }];
}
...
- (void)setCityArray:(NSMutableArray *)cityArray
{
    ...
    [tableView reloadData];
    _areaScrollView.contentSize = CGSizeMake(ScreenWidth * MULPITLE * 2, 300 * MULPITLE);
    [UIView animateWithDuration:0.5 animations:^{
        _areaScrollView.contentOffset = CGPointMake(ScreenWidth * MULPITLE, 0);
    }];
}
- (void)setRegionsArray:(NSMutableArray *)regionsArray
{
    ...
    [tableView reloadData];
    _areaScrollView.contentSize = CGSizeMake(ScreenWidth * MULPITLE * 3, 300 * MULPITLE);
    [UIView animateWithDuration:0.5 animations:^{
        _areaScrollView.contentOffset = CGPointMake(ScreenWidth * 2 * MULPITLE, 0);
    }];
}
    ...
    switch (tableView.tag - 200) {
        case 0:
        {
            ...
            if ([self.address_delegate respondsToSelector:@selector(selectIndex:selectID:)]) {
                [self.address_delegate selectIndex:1 selectID:addressAreaModel.value];
            }
        }
            break;
        case 1:
        {
            ...
            if ([self.address_delegate respondsToSelector:@selector(selectIndex:selectID:)]) {
                [self.address_delegate selectIndex:2 selectID:addressAreaModel.value];
            }
        }
            break;
        case 2:
        {
            ...
            lineView3.hidden = NO;
            if (btn1Height + btn2Height + btn3Height > ScreenWidth) {
                btn3Height = ScreenWidth - (btn1Height + btn2Height);
            }
            ...
            if ([self.address_delegate respondsToSelector:@selector(selectIndex:selectID:)]) {
                [self.address_delegate selectIndex:3 selectID:addressAreaModel.value];
            }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (scrollView == _areaScrollView) {
        UIView *lineView = [_areaWhiteBaseView viewWithTag:300 + scrollView.contentOffset.x / (ScreenWidth * MULPITLE)];
        lineView.hidden = NO;
    }
...
- (void)showAreaView
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        _areaWhiteBaseView.frame = CGRectMakes(0, ScreenHeight - 420, ScreenWidth, 420);
    }];
}
...
- (void)hidenAreaView
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        blackBaseView.alpha = 0;
        _areaWhiteBaseView.frame = CGRectMakes(0, ScreenHeight, ScreenWidth, 420);
