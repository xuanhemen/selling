...
#import "CustomerFollowUpModel.h"
...
#import "CustomerFollowUpCell.h"

#import "SLAPP-Swift.h"
#import "HYFollowFiltrateV.h"


@interface CustomerFollowUpVC ()<SDTimeLineCellDelegate, UITextFieldDelegate,ChatKeyBoardDataSource,ChatKeyBoardDelegate>
...
- (void)addBtnClick{
    ...
    [vc configTypeWithT:0];
    vc.proId = self.customerId;
...
- (void)configData{
    ...
    [self showOCProgress];
    [LoginRequest getPostWithMethodName:client_followup params:@{@"id":self.customerId,@"token":model.token,@"class_id":self.currentType} hadToast:true fail:^(NSDictionary<NSString *,id > * _Nonnull a) {
...
- (NSArray *)creatModelsWithDataArray:(NSArray *)data{
    ...
    for (NSDictionary *dic in data) {
        ...
        CustomerFollowUpModel *model = [self getFollowupDetailModel:dic];
...
//        }
    ...
    if (!cell.moreButtonClickedBlock) {
        [cell setMoreButtonClickedBlock:^(NSIndexPath *index) {
            CustomerFollowUpModel *model = weakSelf.dataArray[index.row];
...
//    [LoginRequest getPostWithMethodName:client_new_message_num params:params hadToast:false fail:^(NSDictionary<NSString *,id > * _Nonnull a) {
//    } success:^(NSDictionary<NSString *,id> * _Nonnull a) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [self.tableView cellHeightForIndexPath:indexPath model:model keyPath:@"model" cellClass:[CustomerFollowUpCell class] contentViewWidth:[self cellContentViewWith]];
}
...
-(SDTimeLineCellModel *)getFollowupDetailModel:(NSDictionary *)dic{
    ...
    model.iconName = [NSString stringWithFormat:@"%@",dic[@"head"]];
    model.createname = [NSString stringWithFormat:@"%@",dic[@"createname"]];
...
//    model.client_id = [NSString stringWithFormat:@"%@",dic[@"client_id"]];
    ...
    model.createuserid = [dic[@"createuserid"] toString];
    //
    ...
    for (NSDictionary *subDic  in dic[@"support"]) {
        if ([subDic[@"userid"] isNotEmpty] && [subDic[@"userid"] integerValue] != 0) {
            ...
            lmodel.userName = [subDic[@"realname"] toString];
    ...
    for (NSDictionary *subDic in dic[@"comments"]) {
        ...
        if ([subDic[@"create_userid"] isNotEmpty]) {
            commentItemModel.firstUserName = subDic[@"create_realname"];
            commentItemModel.firstUserId = subDic[@"create_userid"];
        }
        
        if ([subDic[@"to_userid"] integerValue] != 0) {
            commentItemModel.secondUserName = subDic[@"to_realname"];
            commentItemModel.secondUserId = subDic[@"to_userid"];
        }
        
        commentItemModel.commentString = [NSString base64DecodeString:[NSString stringWithFormat:@"%@",subDic[@"encode_content"]]];
        commentItemModel.id = subDic[@"id"];
        commentItemModel.fo_id = subDic[@"fo_id"];
