...
- (void)configTheme{
    ...
    .LeeAddTintColor(DAY , [UIColor groupTableViewBackgroundColor])
    ...
    .LeeAddBackgroundColor(DAY , UIColorFromRGB(0xDDDDDD))
    .LeeAddBackgroundColor(NIGHT , UIColorFromRGB(0xDDDDDD));
...
- (void)setCommentItemsArray:(NSArray *)commentItemsArray
{
    ...
    for (int i = 0; i < needsToAddCount; i++) {
        ...
        label.lee_theme
        .LeeAddTextColor(DAY , UIColorFromRGB(0x555555))
        .LeeAddTextColor(NIGHT , [UIColor grayColor]);
...
//        //NSLog(@"%@--$%@--%@",model.firstUserName,model.secondUserName,model.commentString);
...
//             //NSLog(@"%@-----hahaha",model.attributedContent);
        }
...
- (void)setupWithLikeItemsArray:(NSArray *)likeItemsArray commentItemsArray:(NSArray *)commentItemsArray
{
    ...
    for (int i = 0; i < self.commentItemsArray.count; i++) {
        ...
        .topSpaceToView(lastTopView, topMargin+0)
        .autoHeightRatio(0);
...
- (NSMutableAttributedString *)generateAttributedStringWithCommentItemModel:(SDTimeLineCellCommentItemModel *)model
{
    ...
    if ([model.secondUserName isNotEmpty]) {
        text = [text stringByAppendingString:[NSString stringWithFormat:@"回复%@", model.secondUserName]];
    }
    
    DLog(@"%@-评论",model.commentString);
//    NSString *transString = [NSString base64DecodeString:model.commentString];
    NSString *transString = model.commentString;
//    DLog(@"cc--------- %@ --------%@",transString,model.commentString);
//    NSString *transString = model.commentString;
//    [NSString stringWithFormat:@"%@",[[model.commentString stringByReplacingOccurrencesOfString:@"+" withString:@"%20"]stringByRemovingPercentEncoding]];
    text = [text stringByAppendingString:[NSString stringWithFormat:@"：%@", transString]];
...
//    //NSLog(@"%@---%@----%@",text,model.firstUserName,[text rangeOfString:model.firstUserName]);
    if ([model.secondUserName isNotEmpty]) {
//        //NSLog(@"hhhhh------*-%@-------------$%@",model.secondUserName,model.secondUserId);
...
- (void)didClickLink:(MLLink *)link linkText:(NSString *)linkText linkLabel:(MLLinkLabel *)linkLabel
{
    if (self.didClickPerson) {
        self.didClickPerson(link.linkValue);
    }
    //NSLog(@"%@", link.linkValue);
   
}

- (void)didClickOtherLink:(MLLink *)link linkText:(NSString *)linkText linkLabel:(MLLinkLabel *)linkLabel{
//    //NSLog(@"-----------------------------------");
    
    if (linkLabel == _likeLabel) {
        return;
    }
    
    if (self.didClickCommentWithModel){
        self.didClickCommentWithModel(linkLabel.model);
    }
}
