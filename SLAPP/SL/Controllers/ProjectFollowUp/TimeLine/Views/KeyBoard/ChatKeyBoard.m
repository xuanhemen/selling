...
{
    CGRect frame = CGRectMake(0, 0, kScreenWidth, kChatKeyBoardHeight);
//    if (translucent) {
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    else
    {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            //由于ios11 begin.size.height == 0,此处取消begin.size.height>0的判断
...
{
    if ([self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectFaceBtnClick:)]) {
        [self.delegate chatKeyBoard:self didSelectFaceBtnClick:select];
    }
    if (select && change == NO)
    {
        self.morePanel.hidden = YES;
 ...
 *  @param change  键盘是否弹起
 */
- (void)chatToolBar:(ChatToolBar *)toolBar cameraBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if ([self.delegate respondsToSelector:@selector(chatKeyBoard:didSelectPicBtnClick:)]) {
        [self.delegate chatKeyBoard:self didSelectPicBtnClick:nil];
    }
    if (select && change == NO)
    {
//        self.morePanel.hidden = NO;
...
/**
  ...
  *  @param change  键盘是否弹起
  */
- (void)chatToolBar:(ChatToolBar *)toolBar atBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if ([self.delegate respondsToSelector:@selector(chatKeyBoard:atBtnDidClick:)]) {
        [self.delegate chatKeyBoard:self atBtnDidClick:nil];
    }
    if (select && change == NO)
    {
//        self.morePanel.hidden = NO;
...
//发送按钮
- (void)chatToolBar:(ChatToolBar *)toolBar sendBtnPressed:(BOOL)select
{
...
//删除图片按钮
- (void)chatToolBar:(ChatToolBar *)toolBar delBtnPressed:(UIButton *)button
{
    if ([self.delegate respondsToSelector:@selector(chatKeyBoard:delPicBtnDidClick:)]) {
        [self.delegate chatKeyBoard:self delPicBtnDidClick:button];
    }
...
- (void)setAllowCamera:(BOOL)allowCamera{
    
    self.chatToolBar.allowCamera = allowCamera;
}

- (void)setAllowPic:(BOOL)allowPic
{
    self.chatToolBar.allowPic = allowPic;
}
...
- (void)deleteBackward:(NSString *)text appendText:(NSString *)appendText
{
    if (IsTextContainFace(text)) { // 如果最后一个是表情
        ...
        if (text.length >= 2) {
            NSString *delName = [text componentsSeparatedByString:@"@"].lastObject;
            if (self.sendersArr.count > 0) {
                for (int i = 0; i < self.sendersArr.count; i++) {
                    MemberModel *tempModel = self.sendersArr[i];
                    if ([delName isEqualToString:tempModel.name]) {

                        NSString *senderName = [NSString stringWithFormat:@"@%@",tempModel.name];
                        NSString *current = [text substringToIndex:text.length - senderName.length];

                        [self.chatToolBar setTextViewContent:[current stringByAppendingString:appendText]];
                        self.chatToolBar.textView.selectedRange = NSMakeRange(current.length, 0);
                        [self.sendersArr removeObjectAtIndex:i];
                        return;
                    }
                }
            }
...
{
    if (!_chatToolBar) {
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(0, kScreenHeight - (kFacePanelHeight + kChatToolBarHeight), kScreenWidth, kChatToolBarHeight)];
