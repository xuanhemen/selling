...
//    _facePickerView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth-40, 40)];
    _facePickerView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 40)];
    [self addSubview:_facePickerView];
    
    _sendBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    _sendBtn.frame = CGRectMake(kScreenWidth-40, 0, 40, 40);
...
- (void)loadfaceThemePickerSource:(NSArray *)pickerSource
{
    for (int i = 0; i<pickerSource.count; i++) {
        ...
        if (i == 0) {
            btn.selected = YES;
            [btn setBackgroundColor:[UIColor colorWithRed:220/255.f green:220/255.f blue:220/255.f alpha:1.0f]];
        }
        btn.tag = i+100;
//        [btn setTitle:themeM.themeDecribe forState:UIControlStateNormal];
//        [btn setTitle:themeM.themeDecribe forState:UIControlStateSelected];
//        [btn setTitleColor:[UIColor purpleColor] forState:UIControlStateSelected];
//        [btn setBackgroundColor:kChatKeyBoardColor];
        [btn setImage:[UIImage imageNamed:themeM.themeIcon] forState:UIControlStateNormal];
        [btn setImage:[UIImage imageNamed:themeM.themeDecribe] forState:UIControlStateSelected];
...
//        CGFloat btnW = MAIN_SCREEN_WIDTH/4.0;
        CGFloat btnW = 50;
...
{
//    [_facePickerView setContentOffset:CGPointMake(subjectIndex*40, 0) animated:YES];
