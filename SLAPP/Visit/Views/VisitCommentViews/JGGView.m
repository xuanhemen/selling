...
-(void)JGGView:(JGGView *)jggView DataSource:(NSArray *)dataSource completeBlock:(TapBlcok)tapBlock
{
        for (NSUInteger i=0; i<dataSource.count; i++) {
            ...
            if ([dataSource[i] isKindOfClass:[UIImage class]]) {
                ...
                [iv sd_setImageWithURL:[NSURL URLWithString:dataSource[i]] placeholderImage:[UIImage imageNamed:@"defaultImage"]];
            }else if ([dataSource[i] isKindOfClass:[NSURL class]]){
                [iv sd_setImageWithURL:dataSource[i] placeholderImage:[UIImage imageNamed:@"defaultImage"]];
            }
...
{
    if ([self.dataSourceBig isNotEmpty] && (index <= self.dataSourceBig.count ))
    {
        NSString *urlString = self.dataSourceBig[index];
